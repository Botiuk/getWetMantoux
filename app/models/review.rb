...
class Review < ApplicationRecord
  ...
  def self.close_records_ids
    @close_record_ids = Review.joins(:rich_text_recommendation)
                              .where.not(rich_text_recommendation: { body: [nil, ''] }).pluck(:record_id)
  end

  def self.reviews_index_admin
    Review.close_records_ids
    Review.unscoped.where(review_date: Time.zone.today..(Time.zone.today + 7))
          .where.not(id: @close_record_ids).order_by_doctors
  end

  def self.reviews_index_doctor(doctor_id)
    Review.close_records_ids
    Review.where(doctor_id: doctor_id, review_date: Time.zone.today..(Time.zone.today + 7))
          .where.not(id: @close_record_ids)
  end

  def self.reviews_index_user(user_id)
    Review.close_records_ids
    Review.where(user_id: user_id, review_date: Time.zone.today..(Time.zone.today + 7))
          .where.not(id: @close_record_ids)
  end
  ...
  def self.open_review_for_pair(doctor_id, user_id)
    Review.close_records_ids
    Review.where(doctor_id: doctor_id, user_id: user_id, review_date: Time.zone.today..(Time.zone.today + 7))
          .where.not(id: @close_record_ids).pluck(:id)
  end
end
